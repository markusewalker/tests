...
func (d *DeleteRKE1ClusterTestSuite) SetupSuite() {
	...
	d.provisioningConfig.NodePools = nodeRolesStandard

	d.rke1ClusterID, err = resources.ProvisionRKE1Cluster(d.T(), d.standardUserClient, d.provisioningConfig, true, false)
	require.NoError(d.T(), err)
}
...
func (d *DeleteRKE1ClusterTestSuite) TestDeletingRKE1Cluster() {
	tests := []struct {
		...
		{"Deleting RKE1 cluster", d.rke1ClusterID},
	}
