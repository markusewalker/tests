...
func (s *AutoReplaceSuite) SetupSuite() {
	...
	s.clusterConfig.MachinePools = nodeRolesStandard

	s.rke2ClusterID, err = resources.ProvisionRKE2K3SCluster(s.T(), s.standardUserClient, extClusters.RKE2ClusterType.String(), s.clusterConfig, true, false)
	require.NoError(s.T(), err)

	s.k3sClusterID, err = resources.ProvisionRKE2K3SCluster(s.T(), s.standardUserClient, extClusters.K3SClusterType.String(), s.clusterConfig, true, false)
	require.NoError(s.T(), err)
}
...
func (s *AutoReplaceSuite) TestAutoReplace() {
	tests := []struct {
		...
		{"Auto replace RKE2 etcd", "etcd", s.rke2ClusterID},
		{"Auto replace RKE2 controlplane", "control-plane", s.rke2ClusterID},
		{"Auto replace RKE2 worker", "worker", s.rke2ClusterID},
		{"Auto replace K3S etcd", "etcd", s.k3sClusterID},
		{"Auto replace K3S controlplane", "control-plane", s.k3sClusterID},
		{"Auto replace K3S worker", "worker", s.k3sClusterID},
	}
