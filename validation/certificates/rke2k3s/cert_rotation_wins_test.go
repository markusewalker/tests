...
func (c *CertRotationWindowsTestSuite) SetupSuite() {
	...
	c.clusterConfig = new(clusters.ClusterConfig)
	operations.LoadObjectFromMap(defaults.ClusterConfigKey, c.cattleConfig, c.clusterConfig)

	if c.clusterConfig.Provider != "vsphere" {
	...
	c.clusterConfig.MachinePools = nodeRolesStandard

	c.rke2ClusterID, err = resources.ProvisionRKE2K3SCluster(c.T(), c.standardUserClient, extClusters.RKE2ClusterType.String(), c.clusterConfig, true, false)
	require.NoError(c.T(), err)
}
...
func (c *CertRotationWindowsTestSuite) TestCertRotationWindows() {
	tests := []struct {
		...
		{"RKE2 Windows cert rotation", c.rke2ClusterID},
	}
