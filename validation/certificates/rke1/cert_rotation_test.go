...
func (c *RKE1CertRotationTestSuite) SetupSuite() {
	...
	c.provisioningConfig.NodePools = nodeRolesStandard

	c.rke1ClusterID, err = resources.ProvisionRKE1Cluster(c.T(), c.standardUserClient, c.provisioningConfig, true, false)
	require.NoError(c.T(), err)
}
...
func (c *RKE1CertRotationTestSuite) TestRKE1CertRotation() {
	tests := []struct {
		...
		{"RKE1 cert rotation", c.rke1ClusterID},
	}
